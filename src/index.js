/* 
  A forked version of gridsome-plugin-algolia: https://github.com/u12206050/gridsome-plugin-algolia

  This script has been converted from a Gridsome plugin 
  to an ES6 Module to be run as a postbuild process

  The algoliasearch API methods in this script have been 
  updated to conform to the Version 4 library and has removed the 
  deprecated methods which are contained in the original plugin.
  Also Promise.allSettled methods have been used to ensure all 
  jobs are completed before processing which would previously 
  cause intermittant errors
*/

import { collections } from '../../../algolia.config.js'
import algoliasearch from 'algoliasearch';
import chunk from 'lodash.chunk';

const graphql = async (query) => {
  return await fetch(process.env.CRAFT_API_URL, {
    method: "POST",
    headers: {
...
const APP_ID = process.env.GRIDSOME_ALGOLIA_APP_ID;
...
function fetchAlgoliaObjects(index, attributesToRetrieve = ['modified']) {
  return new Promise((resolve, reject) => {
    ...
    const hits = {};
    const browser = index.browseObjects({query: '', attributesToRetrieve,  batch: content => {
...
export const onPostBuild = async ({inputs}) => {

  if (!collections) {
    console.error("Required algolia.config.js configuration file is missing or invalid");
    return false;
  }
  ...
  const jobs = collections.map(async (
    ...
    const chunkJobs = chunks.map(async function(chunked) {
      const { taskID } = await indexToUse.saveObjects(chunked, {
